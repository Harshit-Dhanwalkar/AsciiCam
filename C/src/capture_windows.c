...
static IMFActivate *find_device(const char *device) {
  ...
  for (UINT32 i = 0; i < count; i++) {
    ...
    if (SUCCEEDED(devices[i]->lpVtbl->GetAllocatedString(
            ...
            &name_len))) {
      char narrow[256];
      int n = WideCharToMultiByte(CP_UTF8, 0, name, -1, narrow, sizeof(narrow),
                                  NULL, NULL);
      (void)n;
...
// Walk native media types on stream 0 and pick closest to requested
// width/height
static int negotiate_format(IMFSourceReader *reader, int want_w, int want_h,
  ...
  want_type->lpVtbl->SetUINT64(want_type, &MF_MT_FRAME_SIZE,
